(function () {
  ...
  async function init() {
    ...
    function render() {
      ...
      matched.forEach(book => { const row=document.createElement("article"); row.className="browse-book"; const cover=document.createElement("div"); cover.className="browse-cover"; if(book.cover){const img=document.createElement("img");img.src=book.cover;img.alt="Cover of "+book.title;img.loading="lazy";img.style.cssText="width:100%;height:100%;object-fit:contain;border-radius:6px";cover.appendChild(img)}else cover.textContent="BOOK COVER"; const info=document.createElement("div"),heading=document.createElement("h2"),meta=document.createElement("p"); heading.textContent=book.title; meta.textContent=[categoryNames[book.category]||book.category,query.classValues(book).join(", "),book.series,book.displaySubject||book.subject,book.type||book.bookType].filter(Boolean).join(" · "); info.append(heading,meta); const link=document.createElement("a"); link.className="browse-view"; const linkParams=new URLSearchParams({id:book.id,returnTo:currentBrowseUrl()}); if(bootstrap&&bootstrap.requestedSource==="supabase")linkParams.set("catalogueSource","supabase"); link.href="book-details.html?"+linkParams.toString(); link.textContent="View Book →"; row.append(cover,info,link); results.appendChild(row); });
    }
