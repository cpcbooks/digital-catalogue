  ...
  async function init() {
    ...
    function render() {
      ...
      matched.forEach(book => {
        ...
        const linkParams = new URLSearchParams({ id: book.id });
        if (bootstrap && bootstrap.requestedSource === "supabase") linkParams.set("catalogueSource", "supabase");
        link.href = "book-details.html?" + linkParams.toString();
